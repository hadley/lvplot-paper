...
# @keyword internal 
outputLVplot <- function(x,qu,k,out,depth,alpha, weight=NULL) {
...
if (length(weight) == 0) {
  ...
  conf <- cbind(c(y[rev(low[-1])],y[n-high]),c(y[rev(high[-1])],y[n-low]))
	...
	n <- sum(weight)
	y <- xtabs(weight~x)
	order <- c(rev(low[-1]), n-high, rev(high[-1]), n-low)
	ll <- pmax(floor(order),1)
	hh <- pmin(low+1, n)
	allq <- approx(cumsum(y), as.numeric(names(y)), xout = c(ll, hh), method = "constant", 
            f = 1, rule = 2)$y
	order <- order%%1
	d <- length(order)
	qu <- (1 - order) * allq[1:d] + order * allq[-(1:d)]

	
	conf <- cbind(qu[1:(d/2)],qu[-(1:(d/2))])
}
