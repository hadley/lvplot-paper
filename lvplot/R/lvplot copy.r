...
#X }
LVboxplot.formula <- function(formula,alpha=0.95, k=NULL, perc=NULL, weight=NULL, horizontal=TRUE,col="grey",...) {
...
if (length(weight)==0) {
	...
	low <- pmax(floor(d), 1)
   high <- pmin(low + 1, n)
	allq <- approx(cumsum(wts), as.numeric(names(wts)), xout = c(low, high), method = "constant", 
            f = 1, rule = 2)$y
	order <- d%%1
	qu <- (1 - order) * allq[1:length(d)] + order * allq[-(1:length(d))]
}	   
#pc2 <- proc.time()
#print(pc2-pc1)
...
#browser()		  
	   drawLVplot(xx,pt,k,out,qu,horizontal,col,...)
	 	result[[pt]] <- outputLVplot(xx,qu,k,out,depth,alpha, wtd)      
	   pt <- pt+1
    }
