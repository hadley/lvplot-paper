...
Since John Tukey introduced box plots \citet{tukey72}, various 
...
\citep{vase} acknowledged, these later displays depend 
...
The conventional boxplot \citet{eda, emerson83}  labels an observation as ``outside'' if it lies 
...
(cf.~\citep{dchlv}) %%% cite Ch2 in UREDA
which can be quite large when $n$ is large.  
\citep[1148]{dchbox} demonstrated that the
...
of bytes transferred during the session (see \citep{kw06}
...
approximation in \citep[34]{dchlv} for the expected 
...
The approach of \citep{dchbi}, which
...
``spread''), and ``pseudo-sigma'' citet[40]{dchlv}, 
...
in the middle'';  \citet[457]{tukey60} ; and (b) adjacent
...
session (cf.~\citep{kk06}, for a similar plot).
