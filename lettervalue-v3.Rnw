...
For very small number of values boxplots start to deteriorate -- while it is possible to draw a boxplot based on fewer than five points, the information gained from statistics such as the quartiles is highly suspect.

<<taxi-data, echo=FALSE, cache=TRUE, results='hide'>>=
#ot <- read.csv("code-data/On_Time_On_Time_Performance_2015_1.csv.gz")
data(ontime)
ot <- ontime
...
letter-value plot of the same taxi in and out times of Figure~\ref{fig:taxi-bp}. The different number of values in each group results in the use of different number of letter values. For better comparison across groups, every fourth letter value box is filled by a contrasting hue.
...
@
  ...
  for $t$ distributions on 2, 3, 9 degrees of freedom. Top row:  Conventional boxplots. Bottom row: Letter-value
  plots. }
  \label{t-dist}
\end{figure}
...
#census <- read.csv("code-data/counties.csv")
data(census)
...
The different rules provide the user with choices depending on the desired precision in the letter values shown. Our current implementation of the letter-value plot display uses rule 3 based on a 95\% confidence level as a default.

\section{Examples}\label{examples}
This first example demonstrates the behavior of letter value boxplots in situations with contaminated/heterogenous data.
The plots in Figure~\ref{fig:xpl:histogram} give an overview of the situation:  starting with a sample of size 1,500 from a standard normal distribution, $N(0,1)$, we replace values randomly by a set of points sampled  from $N(4,1)$. We start with a contamination of 1\% (of the original sample size $n$) and end at  25\%, which gives an essentially bimodal distribution. Figures \ref{fig:xpl:boxplot} and \ref{fig:xpl:lvplot} show a comparison of boxplots versus letter value plots for this data.
...
<<sim-xpl-box, dependson='simu-data', echo=FALSE, fig.width=10, fig.height=5, out.width='0.8\\linewidth'>>=
...
<<sim-xpl-lv, dependson='simu-data', echo=FALSE, fig.width=10, fig.height=5.5, out.width='0.8\\linewidth', warning=FALSE>>=
par(mar=c(4,2,1,5))
