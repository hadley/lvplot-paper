...
that have gained widespread adoption. Despite \hh{being commonly} used, they are
not altogether satisfactory, particularly for large \hh{and very small} data sets. Specifically,
...
Figure~\ref{fig:internet-bp} illustrates both \hh{large-data} problems with a
...
<<sessions-data, echo=FALSE, cache=TRUE, results='hide'>>=
sessions <- read.csv("code-data/sessions.csv")
...
\begin{figure}[hbtp]
  \centering
<<fig-session-1, dependon='session-data', echo=FALSE, fig.width=10, fig.height=5, out.width='\\textwidth'>>=
...
\begin{figure}[hbtp]
  \centering
<<fig-session-2, dependon='session-data', echo=FALSE, fig.width=10, fig.height=5, out.width='\\textwidth'>>=
