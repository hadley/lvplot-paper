...
Alternative displays have been proposed to illustrate further details of the distribution, such as multi-modality. The overview paper by \citet{wickham:2012} discusses these alternatives, in particular
...
Not surprisingly, all airline carriers seem to be affected similarly by delays due to National Airline Security (NAS), as we can see from the letter value boxplots in the middle of Figure~\ref{fig:xpl:delays}. There are some exceptions: Hawaiian Airlines has almost no problems due to NAS delays, Southwest (WN) and Virgin America  (VX) have a median of zero delay due to NAS, whereas the small carrier Spirit Airlines (NK)  seems to be affected by NAS delays often and long.
...
\item[R-package lvplot:] R-package lvplot consists of the implementation of letter-value plots as described in the paper. The package was used for all letter-value plots in the paper and is available from CRAN. (GNU zipped tar file)
