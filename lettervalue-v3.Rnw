...
\begin{proof} for (iii):
 Consider the upper $k^{th}$ letter value, $LV_k$.
The $k$th letter value is approximately the median of $2d_k$ values, its $(1-\alpha)100$\% confidence interval therefore encompasses approximately $\sqrt{2d_k} z_{1-\alpha/2}$ values. 
