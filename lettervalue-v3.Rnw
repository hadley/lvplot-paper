...
\author{\begin{tabular}[t]{c c c }
  ...
  Iowa State University & Rice University       & University of Virginia  \\
...
\end{equation}
%
\hh{Note that for small sample sizes    the letter value plot simplifies under this rule to a dotplot ($n < 8$) or a dotplot with a median ($8 \le n < 16$).}
...
\noindent In effect, conventional boxplots use this rule \hh{for outlier identification}, with $p = 0.007$.
...
\hh{XXX all of these numbers were off by a factor of ten}
...
\end{equation}
\hh{Interestingly, using a 95\% confidence interval, this rules leads to } labeling 5--8 of the most extreme observations on each side, surprisingly consistent with many of the displays in \citet{eda}.
...
The different rules provide the user with choices depending on the desired precision in the letter values shown. Our current implementation of the letter-value plot display uses rule 3 \hh{based on a 95\% confidence level }as a default.
...
<<simu-data, echo=FALSE, results='hide'>>=
