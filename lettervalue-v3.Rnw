...
<<boxplots, echo=FALSE, fig.width=6, fig.height=2, out.width='\\linewidth'>>=
...
@
  ...
  \caption{Standard boxplots (top row) and letter-value plots (bottom row) for
...
\noindent The third stopping rule has the obvious advantage that it provides a simple, distribution-free solution. \hh{XXXX sample size dependency needs to be re-worded}
Neither the overall sample size, nor any distribution-related characteristic such as skewness or kurtosis, affects the rule. When $\alpha = 0.05$ (95\% point-wise confidence), the rule leads to showing only those letter values whose depths are at least 10 (i.e., labeling 5--8 observations on each side).
