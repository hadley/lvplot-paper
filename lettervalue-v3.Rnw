...
\bigskip
\begin{abstract}
Conventional boxplots~\citep{eda} are useful displays for conveying rough information about the central 50\% and the extent of data. For small-sized data sets, detailed estimates of tail behavior beyond the quartiles may not be trustworthy, so the information provided by boxplots is appropriately somewhat vague, and the expected number of ``outliers" is often less than 10~\citep{dchbox}. Larger data sets afford more precise estimates of quantiles beyond the quartiles; but boxplots do not show this information about the tails, instead presenting large numbers of extreme, though not unexpected, observations.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
\begin{subfigure}[t]{\textwidth}
  \caption{Letter-value plots with increasing amount of contamination.}
   \centering
...
%
\begin{subfigure}[t]{\textwidth}
  \caption{Area-adjusted Letter-value plots.}
\centering
