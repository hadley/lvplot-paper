...
Alternative displays have been proposed to illustrate further details of the distribution, such as multi-modality. The overview paper by \citet{wickham2012} discusses these alternatives, in particular
 vase~\citep{vase}, violin~\citep{violin},  box-percentile
\citep{box.percentiles}, and high-density region~\citep{hyndman1996} box plots. These displays provide more detailed information
...
Our implementation of letter-value plots is available in the R package, \texttt{lvplot}. The online supplementary material contains all code and data used for the plots in this paper.
...
<<sim-xpl-lv, dependson='simu-data', echo=FALSE, fig.width=10, fig.height=5.5, out.width='0.8\\linewidth'>>=
par(mar=c(4,2,1,5))
...
par(mar=c(3,4,0,5))
with(ot, LVboxplot(sqrt(WeatherDelay)~UniqueCarrier, col=cols, horizontal=FALSE,  xlab="Carrier", ylab="Square Root of Weather Delay (in minutes)", median.col="grey90"))
legend( x = 15.25, y= 24.5, legend=1:11, fill=c("grey90", rev(cols)[-(1:4)]),
              title="LV", xpd=TRUE, x.intersp=0.25)
...
Not surprisingly, all airline carriers seem to be affected similarly by delays due to National Air System (NAS), as we can see from the letter value boxplots in the middle of Figure~\ref{fig:xpl:delays}. There are some exceptions: Hawaiian Airlines has almost no problems due to NAS delays, Southwest (WN) and Virgin America  (VX) have a median of zero delay due to NAS, whereas the small carrier Spirit Airlines (NK)  seems to be affected by NAS delays often and long.
Weather delays, as shown in the bottom row of Figure~\ref{fig:xpl:delays} affect different airline carriers differently, most likely due to different routes serviced. Haiwaiian Airlines (HA) is not likely to be affected by inclement weather in January, and therefore delays due to weather are minimal. Envoy Air (MQ, formerly American Eagle Airlines), on the other hand, is affected the most by weather delays.
...
Using location depths, introduced by~\citet{tukey75}, letter values could be extended to bivariate situations, which would allow to include letter-values in  bagplots~\citep{bagplots, aplpack}.
...
\item[R-package lvplot:] R-package lvplot consists of the implementation of letter-value plots as described in the paper. Version 0.1.1 of the package was used for all letter-value plots in the paper and is available from CRAN. (GNU zipped tar file)

\item[lettervalue.R:] R code to reproduce all of the figures in the paper. Note that the data has to be stored in a folder called code-data at the same level as the code file. (.R file)
