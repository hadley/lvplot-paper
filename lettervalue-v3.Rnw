...
lv_horiz <- function(x, xlab = "", width.method="linear", ...) {
  LVboxplot(x, xlab = xlab, horizontal = TRUE, col = cols, width.method=width.method)
}
...
ggplot(ontime, aes(UniqueCarrier, TaxiOut + TaxiIn)) +
  ...
  theme_bw()
@
  \caption{Boxplots of (sqrt-transformed) taxi in and out times grouped by
