...
\begin{description}
\item[{\tt linear}:] starting from a maximal width for the median, the boxes for successive letter values  $LV_i$ are decreased linearly in $i$, with the step size determined by the overall width and the highest letter value shown.
